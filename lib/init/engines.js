...
import {
    createSynchronousEnvironment, createSynchronousFilesystemLoader, createSynchronousFunction,
} from "twing";
import fs from "fs";
...
function setMiyagiEngine() {
    ...
    if (global.config.engine.name == "twing" && global.config.engine.instance) {
        ...
        loader = createSynchronousFilesystemLoader(fs);
        twing = createSynchronousEnvironment(loader);
    }
    ...
    twing.addFunction(
        createSynchronousFunction(
            ...
            (_context, item, requestedComponent, requestedVariation) => {
                    ...
                    return tedComponent == item.parentShortPath &&
